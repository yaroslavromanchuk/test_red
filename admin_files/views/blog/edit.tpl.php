...
?>
    ...
    <table id="editpage" cellpadding="5" cellspacing="0" class="table">
	  ...
	  <tr>
        ...
        <td><label>
          <input type="checkbox" name="public" value="1" id="checkbox" <?php echo ($this->blogedit->getPublic()==1) ? 'checked' : '';?>/>
        </label></td>
      </tr>
	  <tr >
        <td class="kolom2">Дата публикации</td>
        <td>
		<input  name="ctime" type="date" class="form-control input" id="ctime" value="<?php echo $this->blogedit->getCtime()?date('Y-m-d', strtotime($this->blogedit->getCtime())):date('Y-m-d')?>"/> 
		</td>
      </tr>
      <tr>
        <td class="kolom2">Автор</td>
        <td><label>
          <input type="text" name="autor" class="form-control input"  id="autor" value="<?=$this->blogedit->getAutor();?>"/>
        </label></td>
      </tr>
	  <tr>
