...
<form action="<?=wsActiveRecord::useStatic('Menu')->findByUrl('basket')->getPath()?>" method="post" id="basket1" class="cart-table1" >
	...
	?>
		...
		<i class="glyphicon ion-md-close" aria-hidden="true" ></i>
				</a>
                            </div>
			...
			<?php } ?>
                      <?php  if($this->ws->getCustomer()->getBonus()){ ?>
						...
						<?=Shoparticles::showPrice($this->ws->getCustomer()->getBonus())?></strong><strong> грн
					</strong>
                                    </div>
                                </div>
                                <div class="row m-2">
                     ...
                     <?php   } ?>
                                ...
                                <div class="row m-2">
                                <div class="col-8"><b><?=$this->trans->get('Сумма скидки')?>:</b></div>
                                <div class="col-4"><b><?=Shoparticles::showPrice($this->card['total_price_minus'])?> грн.</b></div>
                            </div>
                            <div class="row m-2">
                                <div class="col-8"><b><?=$this->trans->get('Всего к оплате')?>:</b></div>
                                <div class="col-4"><b class="val_sum"><?=Shoparticles::showPrice($this->card['total_price'])?> грн.</b></div>
                                <!--<div class="col-12 alert " id="dop_s" style="font-size: 12px;"></div>-->
                            </div>
                            <?php if(true){//$this->ws->getCustomer()->id == 8005
                                   ?>
                                ...
                                <?php if($this->kupon and $this->kupon['flag']){ ?>
                                   <div class="row m-2">
                                <div class="col-8"><b><?=$this->trans->get('Промокод')?>:</b></div>
                                <div class="col-4"><b><?=$this->kupon['cod']?></b><botoom data-tooltip="tooltip" style="padding: 5px;margin-left: 5px;cursor: pointer" onClick='dellKupon(); return false;' data-original-title="Удалить промокод" ><i class="icon ion-ios-close" style="font-size: 1rem"></i></botoom></div>
                            </div> 
                                <?php }else{ ?>
                                <div class="row m-2">
