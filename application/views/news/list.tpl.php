<h1><?=$this->trans->get('Наши Акции')?></h1>
<?=$this->getCurMenu()->getPageBody();?>
<div class="row mx-auto">
<?php
$today = date("Y-m-d H:i:s"); 
$all_news = wsActiveRecord::useStatic('News')->findAll(array("status"=> 2, "start_datetime <= '$today' and '$today' <= end_datetime"),array(),array(4));
if($all_news->count())
foreach($all_news as $news) {?>
<div class="col-sm-12 col-md-6 col-lg-4 col-xl-4 p-1">
<div class="bg-white p-2 h-100 text-center">
	<h2><?=$news->getTitle()?></h2>
	<p style="text-align:justify"><?=$news->getIntro()?></p>
