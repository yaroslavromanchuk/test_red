...
<br/>
<a class="next-step zak" href="/moe_chudo/add/">Добавить фото</a>
<br/>
...
<div class="articles-list">
    ...
    {
        if (!($cnt % 4)) echo '<div class="articles-row" style="padding: 3px 0 0; height: auto;">';
        ...
        ?>
        <div class="article-item" style="height: auto; padding: 10px 8px;">
            ...
            <div class="view_all_text" style="display: none; background: #fff; width: 142px; padding: 5px; border: 1px solid #fff; position: absolute;"><?=$foto->getText()?></div>
                <?=$foto->getItem()?> "<?=$foto->getBrend()?><?=$foto->getText()?>"</div>

            <?php } ?>
            <p class="name" style="height: 12px; overflow: hidden;">Возраст: <?=$foto->getAge()?><?=$foto->getPrice()?>
            </p>
            <p  class="name" style="height: 12px; overflow: hidden;"> Хобби: <?=$foto->getHoby()?></p>
            <p  class="name" style="height: 30px; overflow: hidden;"> Номинация: <?php
                switch($foto->getType()){
                    case '1': echo '«Чудесный мальчик»'; break;
                    case '2': echo '«Чудесная девочка»'; break;
                    case '3': echo '«Самое оригинальное чудо»'; break;
                }
                ?></p
