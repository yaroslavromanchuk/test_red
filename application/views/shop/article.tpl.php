...
<button id="sub_bascet"  onclick="getQuickCart('<?=$this->getShopItem()->getPath()?>'); return false;" class="btn btn-danger btn-lg" data-placement="bottom"  data-tooltip="tooltip"  title="<?=$this->trans->get('Выбраный Вами товар будет добавлен в корзину'); ?>"  >
					<i class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></i><span><?=$this->trans->get('В КОРЗИНУ')?></span>
                                        
			</button><span class="error error_add">
							<i class="arrow-left"></i>
							<span><?=$this->trans->get('Этот товар уже есть в корзине')?></span>
						</span>
                                        <span class="error ok_add">
							<i class="arrow-left"></i>
							<span><?=$this->trans->get('ТОВАР ДОБАВЛЕН В КОРЗИНУ')?></span>
					</span>
</div>
<?php	
