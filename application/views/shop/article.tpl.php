...
<!--foto-->
<div class="photos mx-1 col-xs-12 col-sm-12 col-lg-5">
...
<!--/foto-->
	<div class="texts col-xs-12 col-sm-12 col-lg-5 px-0">
...
<?php
			...
			$ms = 0;
			foreach ($count as $count) { $ms = $ms + $count->getCount();}
			if ($ms > 0) {
?>
		<!--<p>Выбор размер</p>-->
				<div class="buy-form">
...
?>
<div class="buttom_click" >
<span  onclick="submitCartValidator('<?=$this->getShopItem()->getPath(); ?>');ga('send', 'pageview', '/virtual/tovartobacket'); return true;" class="submit article_submit_new"  data-placement="bottom"  data-tooltip="tooltip"  title="<?=$this->trans->get('Выбраный Вами товар будет добавлен в корзину'); ?>">
<i class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></i><span><?=$this->trans->get('В КОРЗИНУ'); ?></span>
</span>
</div>
<?php
