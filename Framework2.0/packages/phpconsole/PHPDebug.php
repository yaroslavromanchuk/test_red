...
function debug($name, $var = null, $type = LOG) {
    ...
    if (!empty($var)) {
        if (is_object($var) || is_array($var)) {
            ...
            switch($type) {
                case LOG:
                    echo 'console.debug(val'.preg_replace('~[^A-Za-zА-Яа-яІіїЇёЁ|0-9]~i',"_",$name).');'.NL;
                break;
                case INFO:
                    echo 'console.info(val'.preg_replace('~[^A-Za-zА-Яа-яІіїЇёЁ|0-9]~i',"_",$name).');'.NL;
                break;
                case WARN:
                    echo 'console.warn(val'.preg_replace('~[^A-Za-zА-Яа-яІіїЇёЁ|0-9]~i',"_",$name).');'.NL;
                break;
                case ERROR:
                    echo 'console.error(val'.preg_replace('~[^A-Za-zА-Яа-яІіїЇёЁ|0-9]~i',"_",$name).');'.NL;
                break;
            }
        } else {
            switch($type) {
