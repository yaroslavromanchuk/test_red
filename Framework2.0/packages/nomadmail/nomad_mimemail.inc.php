...
class nomad_mimemail
{
	...
	function set_to($mail_to, $name = "")
	{
		//if ($this->_validate_mail($mail_to)){
			$this->mail_to = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_to>" : $mail_to;
			return true;
		//}
		return false;
	}
	...
	function set_cc($mail_cc, $name = "")
	{
		if ($this->_validate_mail($mail_cc)){
			$this->mail_cc = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_cc>" : $mail_cc;
			return true;
		}
		return false;
	}
	...
	function set_bcc($mail_bcc, $name = "")
	{
		if ($this->_validate_mail($mail_bcc)){
			$this->mail_bcc = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_bcc>" : $mail_bcc;
			return true;
		}
		return false;
	}
	...
	function set_reply_to($mail_reply_to, $name = "")
	{
		if ($this->_validate_mail($mail_reply_to)){
			$this->mail_reply_to = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_reply_to>" : $mail_reply_to;
			return true;
		}
		return false;
	}
	...
	{
		if ($this->_validate_mail($mail_to)){
			$mail_to = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_to>" : $mail_to;
	...
	{
		if ($this->_validate_mail($mail_cc)){
			$mail_cc = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_cc>" : $mail_cc;
	...
	{
		if ($this->_validate_mail($mail_bcc)){
			$mail_bcc = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_bcc>" : $mail_bcc;
	...
	{
		if ($this->_validate_mail($mail_reply_to)){
			$mail_reply_to = !empty($name) ? mb_encode_mimeheader($name) . " <$mail_reply_to>" : $mail_reply_to;
	...
	{
		$this->mail_subject = mb_encode_mimeheader(!empty($subject) ? trim($subject) : "No subject");
	}
	...
	function _build_header($content_type)
	{
		...
		$this->mail_header .= "Message-Id: <" . md5(uniqid(rand())) . ".r24_mimemail@" . $_SERVER['SERVER_ADDR'] . ">" . BR;
		$this->mail_header .= "MIME-Version: 1.0" . BR;
		$this->mail_header .= "Precedence: list" . BR;
		$this->mail_header .= "X-Mailer: R24 MIME Mail ". $this->get_version() . BR . BR;
	}
	...
	function _parse_elements()
	{
		...
		*/
		...
		if (!empty($this->mail_html)){
			...
			if (empty($this->mail_text)){
				$this->mail_text = strip_tags(str_replace("<br>", BR, $this->mail_html));
				$this->mail_type = $this->mail_type + 1; // Plain Text
			}
		}
		...
		if ($this->attachments_index != 0){
			foreach($this->attachments as $key => $value){
				if (preg_match('/(css|image)/i', $value['type']) && preg_match('/\s(background|href|src)\s*=\s*[\"|\'](' . $value['name'] . ')[\"|\'].*>/is', $this->mail_html)) {
					$img_id = md5($value['name']) . ".r24@mimemail";
