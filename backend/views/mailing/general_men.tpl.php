...
<p>Рассылка будет
    отправлена <?php echo wsActiveRecord::useStatic('Subscriber')->count(array('active' => 1, 'confirmed is not null', 'men'=>1));?>
    подписчикам.</p>
<input type="hidden" id="all_subject" name="all_subject" value=" <?php echo wsActiveRecord::useStatic('Subscriber')->count(array('active' => 1, 'confirmed is not null', 'men'=>1));?>"/>	   
<?php
if ($this->errors) {
    ?>
...
<script type="text/javascript">
	function loadArticles(category_id, i) {
		...
		$.post('/admin/menmailing/', data_to_post, function (data) {
	...
	function createSelectList(data, a, item) {
		if ('done' == data.result) {
			...
			for (var i = 0; i < data.data.length; i++) {
				...
				out += '<option class="articles"  value="' + data.data[i].id + '">' + data.data[i].title + himg + '</option>';
			}
...
<form method="POST" id="mail_form" action="<?php echo $this->path; ?>menmailing/" target="_blank">
