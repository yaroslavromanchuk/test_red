<?php
$dname = Config::findByCode('domain_name')->getValue();?>
<html>
    <head>
	...
	<a href="http://www.facebook.com/pages/RED-UA/148503625241218?sk=wall"><img src="https://<?=$dname.$this->files?>img/email/fb.png" alt="facebook"   style="width:30px;padding: 0px 1px;" ></a>
...
<table border="0" cellpadding="0" cellspacing="0"  style="width:700px;background: white;" align="center">
			<tr>
				<td style="color:#383838; padding:0">
					<p><?php if(@$this->message->intro) echo stripslashes(str_replace('#', '&', $this->message->intro));?></p>
				</td>
			</tr>
			<tr>
				<td style="padding-left: 0%;">
					<?php $i = 0; if(@$this->message->article_id)foreach ($this->message->article_id as $item) {
					++$i;
					if($i==4) echo "<br>";
			...
			<tr>
				<td style="color:#383838; padding:0"><?php if(@$this->message->ending) echo stripslashes(str_replace('#', '&', $this->message->ending));?></td>
			</tr>
</table>
...
<td colspan="3" style="background:#ddd;font-size: 9px;padding:10px; text-align:center;" >
				<span>Вы получили это письмо на адрес <?=$this->email?>, потому что оформили подписку на сайте Red.ua. Если Вы больше не хотите получать эти письма, нажмите </span><a href="/subscribe/?email=<?=$this->email.$this->message->unsubscribe?>"><span style="color:#383838">тут</span></a> <span>для того чтобы отписаться.</span>
				</td>
			</tr>
</table>
</body>
