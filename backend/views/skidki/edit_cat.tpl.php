...
<form method="POST" action="<?php echo $this->path; ?>skidka/cat/<?php echo (int)$this->sub->getId(); ?>/"
      enctype="multipart/form-data">
    <input type="hidden" value="<?php echo $this->sub->getId(); ?>" name="cat_id">
    <table id="editpage" cellpadding="5" cellspacing="0">
        <tr>
            <td class="kolom1">Категория</td>
            <td>
                <a href="<?php ?>" target="_blank"><?php echo Shopcategories::CatName((int)$this->sub->getCategoryId()); ?>
                    </a>
            </td>
        </tr>
        <tr>
            <td class="kolom1">Создал</td>
        ...
        <tr>
            ...
            <td>
                <input name="start" type="date" class="input"
                       value="<?php echo $this->sub->start ? date('Y-m-d', strtotime($this->sub->start)) : date('d.m.Y'); ?>">
        ...
        <tr>
            ...
            <td><input name="finish" type="date" class="input"
                       value="<?php echo $this->sub->finish ? date('Y-m-d', strtotime($this->sub->finish)) : date('d.m.Y'); ?>">
