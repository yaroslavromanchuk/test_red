...
<button onclick="$('#nakladna').val('');return false;" class="btn btn-primary pd-x-20">Завершить</button>
    ...
    <div class="card-body">
        <?php 
if ($this->getOrders()) {
$deli = [ 3 => 'Победа', 5 => 'Строителей', 4=>'УкрПочта', 9=>'Курьер', 8=>'НП', 16=>'НП:НП',18 => 'Justin',]; 
?> 
<script>
...
<table id="orders" class="table table-striped table-sm  table-hover datatable1 dataTable table-dark" >
  ...
  ?>
    ...
    <tr <?php if($order->status == 1 or $order->status == 2){ echo 'style="background: '.$color.';"';} ?> id="<?=$order->getOrderId()?>">
        <td>
 <label class="ckbox"><input type="checkbox" class="order-item cheker" onChange="return Calculat();" name="item_<?=$order->getId()?>"/><span></span></label>
        </td>
		...
		<td><?=$order->priynav->middle_name?></td>
		<td><?=$order->date_obrabotan?></td>
                <td id="s_<?=$order->id?>"><?=$order->amount?></td>
		<td id="d_<?=$order->id?>"><?=$order->dop_suma?></td>
		<td><?=$order->order->getDeliveryTypeId() ? $order->order->getDeliveryType()->getName(): ''?></td>
                <td><?=$order->order->getPaymentMethod() ? $order->order->getPaymentMethod()->getName(): ''?>
        <?=($order->order->deposit>0)?'<i class="icon ion-ios-chatboxes green tx-20 " data-placement="right"  data-tooltip="tooltip" title="" data-original-title="Депозит '.$order->order->deposit.' грн."></i>':''?></td>
		<td><?=$order->sposob?$spos[$order->sposob]:''?></td>
                <td><?=$order->order->fop->name?></td>
