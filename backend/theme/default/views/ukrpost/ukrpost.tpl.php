...
<span class="input-group-btn">
  ...
  <button class="btn bd bg-white tx-gray-600" data-toggle="modal" data-target="#new_group" type="button">
   ...
   <button class="btn bd bg-white tx-gray-600" onclick="window.open('/admin/ukrpost/print/?sticker=group&uuid='+$('.shipmentGroupUuid').val()+'&type=', '_blank'); return false;" type="button">
       <i class="icon ion-ios-print-outline" data-tooltip="tooltip" data-original-title="Друк стікерів групи"> Стікер 100*100(термопринтер)</i>
   </button>
     <?php if(isset($_GET['group'])){ ?>
<button class="btn bd bg-white tx-gray-600" onclick="window.open('/admin/ukrpost/print/?sticker=group&uuid='+$('.shipmentGroupUuid').val()+'&type=SIZE_A4', '_blank'); return false;" type="button">
       <i class="icon ion-ios-print-outline" data-tooltip="tooltip" data-original-title="Друк стікерів групи"> Стікер 100*100(А4)</i>
   </button>
 <?php } ?>
       ...
       <?php foreach ($this->shipments as $v) { ?>
                <tr>
                  <!--  <td><label class="ckbox"><input type="checkbox" class="order-item cheker" onclick="chek_l();" id="<?=$v->uuid?>"  name="<?=$v->uuid?>"><span></span></label></td>-->
                    ...
                    <td><?=$v->deliveryPrice.'/'?><?php if($v->paidByRecipient){ echo 'Одержувач';}else{ echo 'Відправник';} if($v->discounts){ echo '<br><span style="color:red;">Скидка '.$v->discounts[0]->value.' %</span>';} ?></td>
