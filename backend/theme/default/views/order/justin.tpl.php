...
for ($step = 0; $step < $z; $step++) { ?>
    ...
    <tr>
        <td colspan="9" class="tt_border_bottom">
		...
		<span>(044) 224-40-00 | (063) 809-35-29 | (067) 406-90-80</span>
					...
					<strong>Интернет-магазин «RED.UA»</strong><br>E-mail: market@red.ua</strong>
                    </td>
                </tr>
            </table>
        </td>
    ...
    <tr>
		...
		<td colspan="4" rowspan="5"><p class="qr<?=$this->order->getId()?>" style="float: right;"></p></td>
    </tr>
		<tr>
        <td colspan="5">
        ...
        <tr>
        <td colspan="5">
            <i><strong>Оплата: </strong>
                <?=strip_tags($this->order->getPaymentMethod()->getName())?></i>
            <br/>
        </td>
    </tr>
    <tr>
    ...
    $this->order->calculateOrderPrice2(true, true, false);
    $total_price = 0;
    //$to_pay = 0;
   // $to_pay_minus = 0.00;
...
echo $skid_show ? '<span '.$st.'>'.ceil($skid_show).' %</span>' : '';
	...
	<tr>
		<td colspan="7" align="right"><i style="font-weight: bold;">Код на скидку</i></td>
		<td class="border_all border_right" align="right" colspan="2"><i style="font-weight: bold;"><?=$this->getOrder()->getKupon()?></i></td>
	</tr>
	<tr>
		<td colspan="7" align="right"><i style="font-weight: bold;">Скидка по коду</i></td>
...
<?php
		...
		if($this->getOrder()->getBonus() > 0 and false){ ?>
	<tr>
			<td colspan="7" align="right"><i>Бонусная скидка</i></td>
    ...
    <?php } ?>
	<tr>
		<td colspan="7" align="right" class="fnt_size_1">Доставка:</td>
    ...
    <?php if ($this->getOrder()->getDeposit() > 0) { ?>
        <tr>
            <td colspan="7" align="right"><i>Депозит</i></td>
            <td class="border_all border_right" align="right" colspan="2">
    ...
    <?php if ($this->getOrder()->getDopSumma() > 0) { ?>
        <tr>
            <td colspan="7" align="right" ><span class="fnt_size_1">Дополнительная оплата</span><br>
    ...
    <tr>
        ...
        <td colspan="7" class="border_none fnt_size_1" align="right">
    ...
    <tr>
        <td colspan="9" style=" font-size: 9px; padding-top: 15px;padding-bottom: 15px;">
    ...
    <tr>
        <td colspan="9" style=" font-size: 11px; padding-bottom: 30px;">
...
?>
	...
	<?php }
if($z > 1 and Config::findByCode('tov_check_zayava')->getValue()){
	?>
    ...
    <tr>
		...
		<span>(044) 224-40-00 | (063) 809-35-29 | (067) 406-90-80</span>
                    ...
                    <td style="text-align:center;">
                        <strong>Интернет-магазин «RED.UA»<br>E-mail: market@red.ua</strong>
                    </td>
                </tr>
            </table>
        </td>
