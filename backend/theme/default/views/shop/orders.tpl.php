...
location.reload();
    ...
    <table  id="orders" class="table  table-hover">
        ...
        <?php $row = 'row2'; foreach ($this->getOrders() as $order) {
            $admin = [24148=>'Сергей', 36213=> 'Юра', 37075=>'Андрей', 40184=>'Богдан'];
            
            $row = ($row == 'row2') ? 'row1' : 'row2';
            $order_owner =  $order->customer;
            ?><?php if($order->getIsUnitedly() != 1){
                if ($order->getStatus() == 3 and ($order->delivery_type_id == 3 or $order->delivery_type_id == 5) and  $order->getOrderGo() and $order->getOrderGo() != '0000-00-00' and strtotime($order->getOrderGo()) < mktime(0, 0, 0, date("m"), date("d") - 3, date("Y"))){
...
echo '<i class="icon ion-ios-close red tx-25 pd-5 history_pay_status" data-placement="right" data-id="'.$order->getId().'"   data-tooltip="tooltip"  title="'.$order->liqpay_status->name.'"></i>';
  ...
  <input type="text" class="form-control nakladna" aria-describedby="basic-addon1" id="nakladna" name="nakladna" value="<?=$order->getNakladna(); ?>" pattern="[0-9]{5,14}">
	...
	<img style="padding-left: 5px;" class="img_return" src="<?=SITE_URL; ?>/img/icons/help.png" alt="Отследить" data-placement="right"  data-tooltip="tooltip" title="Отследить"  onclick="k_tracking('<?php echo $order->getNakladna();?>');"/>
            <?php }elseif($order->getDeliveryTypeId() == 18 and @$order->getNakladna()){?>
                <img style="padding-left: 5px;" class="img_return" src="/img/icons/help.png" alt="Отследить" data-placement="right"  data-tooltip="tooltip" title="Отследить"  onclick="justin_tracking('<?=$order->getNakladna()?>');"/>
            <?php } ?>
                        <br/><?php  } ?>
                        <?php if (false) {
