...
if(page != null){   
   ...
   //console.log($('#'+page[1]).offset().top+500);
	...
	$('#savepage').on( "click", function () {
       ...
       $('.opdate_deivery_cost').click(function () {
          UpdatePriceDelivery();
        }); 
    ...
    $.get('/page/savedelyveryordr/',"id=" + $('#get_order_id').html() + '&delyvery=' + $('#delivery_type').val() + '&payment=' + $('#payment_method').val(),
				function (res) {
				$("#dely_cost").html(res);
...
<tr>
    ...
    <td><span id="dely_cost"><?=$peresilka?></span> грн <i class="icon glyphicon glyphicon-refresh bleak  tx-20 m-l-3 pd-5 opdate_deivery_cost" alt="Обновить стоимость" data-placement="right" title="" data-tooltip="tooltip" data-original-title="Обновить стоимость"></i>
    </td>
</tr>
<tr>
	<td></td>
...
<form action="" method="post" class="form-horizontal" style="text-align:left;">
  ...
  <?php if($this->getOrder()->getDeliveryTypeId() == 8 or $this->getOrder()->getDeliveryTypeId() == 16 or $this->getOrder()->getDeliveryTypeId() == 4 or $this->getOrder()->getDeliveryTypeId() == 18){ ?>
  	<div class="form-group">
...
return false;
				...
				<td>
					...
					<br><span style="color: #777;"><?=$article_rec->article_db->category->id?$article_rec->article_db->category->getRoutez():''?></span>
	...
	<?php if ($article_rec->getCount() > 0) { echo $skid_show ? '<span '.$st.'>'.$s_uc.' '.$skid_show.'%</span><br><br>' : ''; } ?>
				...
				//	echo print_r($article_rec->getOptions());
                                       ...
                                       // echo data('Y-m-d', strtotime('2019-10-14'));
                                        //$d = data('Y-m-d', strtotime('-2 days'));
					if(!$article_rec->getOptionId() and $article_rec->getOptions()){ 
                                            
					?>
					<br/>
                                        <select name="option_id-<?=$article_rec->getId()?>" onChange="return addAkciya(this);"  class="form-control">
